...
#define HARD_WIDTH 80
#define HARD_HEIGHT 45

#define EASY_MAP_MAX_NUM    2   // 맵 개수
#define NORMAL_MAP_MAX_NUM  2
#define HARD_MAP_MAX_NUM    2
...
#define EASY_MAP_FILE_PATH      "Map\\EasyMap_%d.txt"
#define NORMAL_MAP_FILE_PATH    "Map\\NormalMap_%d.txt"
#define HARD_MAP_FILE_PATH      "Map\\HardMap_%d.txt"
...
unsigned char* LoadMazeText(int mapSelect)
{
    char *mazeMapFilePath[] = {EASY_MAP_FILE_PATH, NORMAL_MAP_FILE_PATH, HARD_MAP_FILE_PATH};
    COORD size[] = {{EASY_WIDTH, EASY_HEIGHT}, {NORMAL_WIDTH, NORMAL_HEIGHT}, {HARD_WIDTH, HARD_HEIGHT}};
    int mapMaxNum[] = {EASY_MAP_MAX_NUM, NORMAL_MAP_MAX_NUM, HARD_MAP_MAX_NUM};
    mapSize = size[mapSelect];
    int mapNum = RandomNumber(mapMaxNum[mapSelect]);
    char openMazeMapPath[25];

    sprintf(openMazeMapPath, mazeMapFilePath[mapSelect], mapNum);   // 읽어올 map 번호 결정

    unsigned char *wall = (unsigned char*)malloc(sizeof(unsigned char) * (size[mapSelect].X*size[mapSelect].Y)); // 1:벽, 0:길
    FILE *map = fopen(openMazeMapPath, "r");
